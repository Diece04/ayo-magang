...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_perusahaan'])) {

    // ambil id dari select (perusahaan_id), bukan 'id'
    $perusahaan_id = intval($_POST['perusahaan_id']);

    // gunakan placeholder PDO yang benar
    $sql  = "UPDATE perusahaan SET status = 'approved' WHERE id = :id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([':id' => $perusahaan_id]); // eksekusi update

    // redirect supaya tidak resubmit form
    header("Location: approve_perusahaan.php?success=1");
    exit;
}
                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <?php if (isset($_GET['success'])): ?>
                            <div class="alert alert-success">Data perusahaan berhasil disimpan.</div>
                        <?php endif; ?>
                        <div class="row">
                            <div class="col-12">
                                <div class="card p-0" style="border-radius: 20px; box-shadow: 0 15px 30px rgba(0,0,0,0.08); overflow: hidden;">

                                    <!-- header card -->
                                    <div class="px-4 py-2" style="background: #696cff; color:#fff;">
                                        <span style="font-weight:600;">Form Approve Perusahaan</span>
                                    </div>

                                    <div class="card-body" style="background:#ffffff;">
                                        <h5 class="mb-4 d-flex align-items-center">
                                            <span>Approve Perusahaan</span>
                                            <span class="flex-grow-1 ms-3" style="height:1px; background:#d3d6de;"></span>
                                        </h5>
                                        ...
                                        <form action="" method="post">
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select" id="perusahaan_id" name="perusahaan_id" required>
                                                    <option value="" disabled selected>-- Pilih perusahaan pending --</option>
                                                    <?php
                                                    $stmt = $pdo->query("SELECT id, nama FROM perusahaan WHERE status = 'pending'");
                                            ...
                                            <button type="submit" name="approve_perusahaan" class="btn btn-primary">
                                                Approve Perusahaan
                                            </button>
                                        </form>
